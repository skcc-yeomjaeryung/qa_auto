...
> **바이블 Phase 01 — 저장소연결**  
> 선행: `00` · 종료 Gate: `URL/Local 경로로 두 저장소 수집 및 Commit 고정`  
> 교보재 원본: `phase_01_repository_connection.md`  
> 규칙: **한 세션 = 이 Phase만** · Gate 실패 시 다음 Phase 금지  
> 완료 보고: `docs/06.완료보고/PHASE-01.md` (템플릿: `docs/05.템플릿/01.Phase완료보고서.md`)  
...
(선택) Design Spec·Excel/CSV를 프로젝트 보조 Evidence로 업로드할 수 있게 한다.
...
- Ignore/Allow 정책
- Secret 마스킹
- (선택) Design Spec / Test Data Sheet 업로드·메타데이터 저장
...
2. 입력 소스:
   ...
   - 인증이 필요한 GitHub URL
   - 이미 내려받은 Local Path
   - (선택) Design Spec 파일: PPT/PDF/이미지 — `DesignSpecArtifact`로 저장 (`docs/03.계약과예시/04.DesignSpec스키마.md`)
   - (선택) Test Data Sheet: Excel/CSV — `TestDataSheet`로 저장 (`docs/03.계약과예시/05.TestDataSheet스키마.md`)
2-1. 보조 파일은 Scenario/INPUT을 확정하지 않는다. grade=`hint`/`candidate` 또는 `reviewRequired`로만 적재한다.
2-2. SVN 등 비지원 VCS는 1차에서 Local Path snapshot으로 대체한다 (정식 SVN 수집기 제외).
...
- (선택) `POST /api/projects/{id}/design-specs`
- (선택) `GET /api/projects/{id}/design-specs`
- (선택) `POST /api/projects/{id}/test-data-sheets`
- (선택) `GET /api/projects/{id}/test-data-sheets`
...
- (선택) Design Spec · Excel/CSV 업로드와 파싱 상태·경고 표시
- **Progress (D-009):** 주 여정 스텝퍼 Type 2 — `프로젝트` → `저장소`(현재) → `시나리오생성` → `시나리오목록` → `테스트실행`  
  수집 중=`progressing`, 완료=`complete`, 실패=`error`  
  상세: [`docs/02.아키텍처/04.진행상태UI.md`](../../02.아키텍처/04.진행상태UI.md)
...
## 보완 Gate (Console 체인 · 2026-08-04)

- [ ] Console에서 **GitHub URL** 등록 UI가 동작한다 (Local Path만으로 대체하지 않음).
- [ ] 모노레포 `subdir` 필드로 FE/BE workspace를 분리한다.
- [ ] 동일 GitHub URL을 FE·BE role로 각각 등록·Sync할 수 있다.
...
- [ ] 프로젝트·저장소 진행 상태가 Figma Progress Kit(Type 2)로 표시된다.
...
- GitLab/Bitbucket/SVN 정식 지원 (Local Path로 대체)
- Design Spec만으로 시나리오·Commit·Endpoint 확정
- Excel 응답값의 자동 HITL 승인
...
`docs/06.완료보고/PHASE-01.md`를 작성하라.
