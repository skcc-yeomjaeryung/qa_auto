...
> **바이블 Phase 08 — 입력값추천**  
> 선행: `03, 07` · 종료 Gate: `정상·경계·오류·업무상태 추천값과 근거 제공`  
> 교보재 원본: `phase_08_input_recommendation.md`  
> 규칙: **한 세션 = 이 Phase만** · Gate 실패 시 다음 Phase 금지  
> 완료 보고: `docs/06.완료보고/PHASE-08.md` (템플릿: `docs/05.템플릿/01.Phase완료보고서.md`)  
...
Component/Input Contract, Backend Validation, 기존 테스트, Fixture,  
(선택) Excel/CSV Test Data Sheet, Design Spec 의미 힌트, 초기 Best Practice Catalog를 결합해  
사용자가 피로하지 않도록 자동 추천값과 배치용 승인 Input Profile을 생성한다.
...
1. 입력원 우선순위:
   ...
   4) Test Data Sheet (Excel/CSV) — 서비스/거래 아이디 키로 조회. 미승인 행은 `reviewRequired`
   5) 초기 Best Practice Catalog
   6) LLM semantic suggestion (+ Design Spec annotation hint, 있을 때만)
   7) 사용자 입력
1-1. Test Data Sheet·Design Spec은 보조다. Fixture/Schema와 충돌하면 코드·Fixture가 우선이며 충돌을 UI에 표시한다.
1-2. Sheet의 응답/기대 필드는 Input Profile 승인 또는 HITL 전까지 hard expected로 확정하지 않는다.
...
- `GenerationPolicy`
- `DataSourceEvidence`
- (선택 연계) `TestDataSheet` / `DesignSpecArtifact` — `docs/03.계약과예시/04~05`
...
- 무제한 fuzzing
- 업무 기대값의 자동 최종 승인
- Excel/Design Spec만으로 식별자·기대값 확정
- Design Spec Vision 결과를 Fixture보다 우선 적용
...
`docs/06.완료보고/PHASE-08.md`를 작성하라.
