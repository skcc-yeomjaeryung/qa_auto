> SSOT 위치: `docs/01.제품과완료기준/03.파일럿완료기준.md` · 원본: `docs/_archive/20260804-prompt-package/00_pilot_definition_of_done.md`
...
### 기본 Console 체인 (파일럿 여정 Gate)

다음 흐름을 Console에서 재현할 수 있어야 한다.

1. 대상 GitHub 모노레포 URL 연결 (예: [bank-of-anthos](https://github.com/GoogleCloudPlatform/bank-of-anthos.git))
2. FE/BE를 **동일 URL + subdir** (또는 Local Path)로 등록·Sync·Commit 고정
3. 「분석·시나리오 생성」원샷 파이프라인으로 분석 자료·시나리오 목록 적재
4. 테스트 시나리오 목록(`/scenarios`)의 **의존관계 그래프** → `view=graph` Interaction Graph UI

관측만 보고하며 Pass/Fail은 HITL. Analyzer가 스택과 맞지 않으면 `missing_data`/`partial`로 남긴다.

### 저장소 및 분석
- Frontend와 Backend Repository를 URL 또는 Local Path로 등록할 수 있다.
- 모노레포는 `subdir`(예: `src/frontend`, `src/ledger/ledgerwriter`)로 role별 workspace를 분리한다.
- 분석은 특정 Commit SHA로 고정된다.
- (선택·Non-blocking) Design Spec·Excel/CSV를 보조 Evidence로 업로드할 수 있다.  
  없어도 인수 시나리오는 코드 경로만으로 성립해야 한다. 있으면 join 상태·`reviewRequired`가 보여야 한다.
